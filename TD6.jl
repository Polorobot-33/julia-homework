...
# ╔═╡ c3d21eb4-26a6-45ca-945c-24bed71e82fa
using PlutoUI, Symbolics, GLMakie

# ╔═╡ 5682a508-0993-40fa-9ae2-214162f81c6c
using Random
...
# ╔═╡ 4ac5b9c0-b782-4254-9da7-58d54241891e
begin
	
struct Polynom{N, T} <: Function
    	coeffs::NTuple{N, T}
end

function (p::Polynom)(x)
    Base.evalpoly(x, p.coeffs)
end
	
end

# ╔═╡ 1375e056-1ef4-42ec-ad71-b93cfd806cbc
P = Polynom((1,))

# ╔═╡ da477a22-2371-4d57-afb0-ca280a726363
md"""
 x = $(@bind x PlutoUI.Slider(0:0.1:10, show_value=true))
"""

# ╔═╡ 708b27a0-52dd-4d9b-b219-395f5ea921db
P(x)

# ╔═╡ a5f37bea-59ad-4511-b68d-790db469eaa0
md"""
	foo renvoie les coefficients du polynôme unitaire dont les racines sont données en entrée (de même degré que le nombre de racines)
"""

# ╔═╡ 50e549ce-930b-4d4e-9086-4219f3274996
function lagrange_polynome(y, xs, i)
	@variables X
	poly = prod((X - x) / (xs[i] - x) for (j, x) in enumerate(xs) if i != j)
	y * poly
end

# ╔═╡ 689c7924-d733-45d6-a2c5-9404b05bf402
function lagrange_interpolation(xs, ys)
	poly = sum(lagrange_polynome(y, xs, i) for (i, y) in enumerate(ys))
	poly = expand(poly)
	@variables X
	Polynom(ntuple(i -> Symbolics.coeff(poly, X^(i-1)), Val(length(xs)+1)))
	#ntuple(i -> Symbolics.coeff(poly, X^(i-1)), Val(length(xs)+1))
end

# ╔═╡ 5dacdef9-98d5-463c-b008-3ab16e4ed8e7
let f = Figure(), ax = Axis(f[1, 1])
	N = 10
	xs = rand(Float64, N) .* 10
	ys = rand(Float64, N) .* 5

	poly = lagrange_interpolation(xs, ys)
	
	xpos = minimum(xs):0.05:maximum(xs)
	ypos = poly.(x for x in xpos)
...
# ╔═╡ a4db8f42-d412-4129-9b03-ee93882e4e17
function chebychev_interpolator(f, a, b, ::Val{N}) where N
	xs = [(a+b)/2 + (b-a)/2 * cos((2*k - 1) / (2*N) * π) for k in 1:N]
	ys = f.(xs)
	poly = lagrange_interpolation(xs, ys)
end

# ╔═╡ bdfaaff4-735f-45c9-9f29-d6aa59402db2
let F = Figure(), ax = Axis(F[1, 1])
	f = (x -> 1 / (25 * x^2))
	N = 10
	...
	polyL = lagrange_interpolation(xs, ys)
	polyC = chebychev_interpolator(f, -1, 1, Val(N))
	
	xpos = -1:0.01:1
	lagrange = lines!(ax, xpos, polyL.(x for x in xpos))
	chebychev = lines!(ax, xpos, polyC.(x for x in xpos))
	points = scatter!(ax, xs, ys, color=:red, marker=:cross)
	reference = lines!(ax, xpos, f.(xpos))
	
	Label(F[0, :], "Comparaison des interpolations de Lagrange et Chebychev")
	Legend(F[1, 2], [lagrange, chebychev, points, reference], ["Lagrange", "Chebychev", "Points pour Lagrange", "Reference"])
	
	F
end

# ╔═╡ 93780952-6803-4e63-af92-80d524b9d1d8
md"""
L'interpolation de Chebychev produit un résultat avec moins d'oscillations, mais a plus de mal à atteindre les valeurs extrêmes comme en 0
"""

# ╔═╡ 3712b013-ef6c-4485-ad63-1db4b5ac7548
md"""
# Exercice 2
"""

# ╔═╡ 81590df2-25da-4e80-81fc-020995a0a175
function dist(r)
	sqrt(sum((x -> x^2).(r)))
end
...
function randomPos(N)
	...
	for i in 1:N
		...
		#push!(x, pos ./ dist(pos)) # normalization
		append!(x, pos ./ dist(pos)) # normalization
	end
...
function energy(r1, r2)
	1 / dist(r2 .- r1)
end

# ╔═╡ 6a1e5f37-1680-46d0-8dd2-999a46c9a388
function energyTot(x, p)
	sum = 0
	#=for electron1 in x
		for electron2 in x
			(electron1 === electron2) || (sum += energy(electron1, electron2))
		end
	end=#
	for i in 1:3:length(x)
		for j in 1:3:length(x)
			...
			electron1 = x[i:i+2]
			electron2 = x[j:j+2]
			sum += energy(electron1, electron2)
		end
	end
	sum
end
...
function myConstraint(res, x, p)
	res .= [dist(x[i:i+2]) for i in 1:3:length(x)]
	#(res .= (pos -> dist(pos)).(x))
end
...
manifest_format = "2.0"
project_hash = "d1be7325b613236dafcddc5a5723a52b142507c8"
